...
def deal_data(conn, addr):
    ...
    cv2.setMouseCallback(window_name, OnMouseMove, param=param) # 鼠标回调函数，用于鼠标事件
...
def get_msg(conn, length):
    try:
        return conn.recv(length) # 不论是客户还是服务器应用程序都用recv函数从TCP连接的另一端接收数据。 接收数据使用
    except socket.error as e:
        print(e)
...
def OnMouseMove(event, x, y, flags, param):
    win32api.SetCursor(win32api.LoadCursor(0, win32con.IDC_ARROW )) # 设置鼠标位置
